...
# date              :20180807
# version           :1.0
# usage             :python training.py -l 1000 -i ./train_example/tr/encode -j ./train_example/val/encode -k ./train_example/test/encode -o ./train_example/models -f 10 -n 100 -d 100 -e 30 -w 5 -m 0 -p 1 -t 0
...
from expectation_pooling import *
...
parser.add_option("-w", "--localWindowSize", action = "store", type = int, dest = "local_window_size",
                                    default=5, help = "the local window size of the local max pooling before the GlobalExpect")
parser.add_option("-m", "--mode", action = "store", type = int, dest = "mode",
                                    default=0, help = "0 or 1, represents the option structure (calculate the expectation over the max or average)")
parser.add_option("-p", "--baseForProbability", action = "store", type = int, dest = "base_for_probability",
                                    default=1, help = "the base to calculate the prob")
parser.add_option("-t", "--trainable", action = "store", type = int, dest = "trainable",
                                    default=0, help="whether or not the base will be trainable")
...
local_window_size = options.local_window_size
mode = options.mode
m = options.base_for_probability
m_trainable = bool(options.trainable)
...
if os.path.isfile(modName):
  ...
  hidden_layers = [
      ...
      GlobalExpectationPooling1D(mode=mode, m_trainable=m_trainable, m_value=m),
